...
public class MappingMatch {
    ...
    public static void addMappingRule(MongoTemplate mongoTemplate) {
        ...
        */
        otherList.add(generateItem(containArray, new String[]{"EUS+FNA", "EUS引导下FNA术", "穿刺置管记录", "操作记录", "历史摘要", "股静脉置管", "穿刺记录", "透析记录", "超声内镜+穿刺术", "超声胃镜+FNA术", "切除术", "引流术"}, "其他", null, 2, 1));
        otherList.add(generateItem(new String[]{"会诊单"}, null, "会诊单", null, 2, 1));
        ...
        hospitalList.add(generateItem(new String[]{"小时内入出院"}, null, "24小时内入出院", null, 2, 2));
        hospitalList.add(generateItem(new String[]{"ICU入室记录"}, null, "ICU入室记录", null, 2, 3));
        hospitalList.add(generateItem(new String[]{"入院", "体检病历", "甲状腺腺癌"}, new String[]{"护理评估", "介绍表", "入院模板", "病情告知", "死亡"}, "入院记录", null, 2, 4));
        //治疗方案
        ...
        subList.add(generateItem(new String[]{"病程", "查房", "转出", "转入", "危急值记录", "危急值报告", "抢救", "拔管记录", "疑难病例讨论记录", "阶段小结", "术后记录"}, null, "病程", null, 2, 1));
        //出院
        ...
        subList.add(generateItem(new String[]{"ICU出室记录"}, null, "ICU出室记录", null, 2, 1));
        subList.add(generateItem(new String[]{"死亡小结"}, null, "死亡小结", null, 2, 2));
        subList.add(generateItem(new String[]{"死亡"}, null, "死亡记录", null, 2, 3));
        subList.add(generateItem(new String[]{"出院小结"}, null, "出院小结", null, 2, 4));
        subList.add(generateItem(new String[]{"出院", "经超声胃镜超声造影记录"}, new String[]{"病程", "查房", "小时入出院"}, "出院记录", null, 2, 5));
        //检查记录
        ...
        subList.add(generateItem(new String[]{"门诊"}, null, "门诊", null, 2, 1));
        //病理
    ...
    private static JSONObject generateItem(Object[] include, Object[] exclude, String mappedValue, List<JSONObject> subType, Integer level, Integer rank) {
        /*
        ...
        item.put("rank", rank);
        item.put("mappedValue", mappedValue);
